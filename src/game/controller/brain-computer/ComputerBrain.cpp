...
Decision attack(const Snapshot &snap) {
  if (!snap.characters.empty() && rand(0, 5) == 0) {
    ...
    return Decision{ActionType::ATTACK_CHARACTER, snap.character.id, attackedId};
  } else if (!snap.structures.empty() && rand(0, 7) == 0) {
    const auto attackedId = fromVec(snap.structures).id;
    return Decision{ActionType::ATTACK_STRUCTURE, snap.character.id, attackedId};
  } else if (!snap.buildings.empty() && rand(0, 15) == 0) {
    const auto attackedId = fromVec(snap.buildings).id;
    return Decision{ActionType::ATTACK_BUILDING, snap.character.id, attackedId};
  } else {
    return Decision{ActionType::SKIP_TURN};
  }
}
